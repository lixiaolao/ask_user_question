...
pub fn default_mcp_tools() -> HashMap<String, bool> {
    ...
    tools.insert(mcp::TOOL_WEIDAO.to_string(), true); // ask_user_question工具默认启用
...
pub fn default_custom_prompts() -> Vec<CustomPrompt> {
    vec![
        ...
        CustomPrompt {
            ...
            description: Some("ask_user_question的另一个工具，请记住".to_string()),
            sort_order: 4,
            created_at: chrono::Utc::now().to_rfc3339(),
            updated_at: chrono::Utc::now().to_rfc3339(),
            r#type: "normal".to_string(),
